...
# (1) Calculate E(x), V(x) & Sharpe Ratio for each LIC
k <- ncol(data)-2
exrtn.temp <- as.numeric(vector())
exp.rtn.sd <- as.numeric(vector())
sd.temp <- as.numeric(vector())
er.list <- names(capm.data)
n <- nrow(er.list)
er.list <- er.list[3:(k+2)]
for(i in 1:k){
  form.temp <- capm.data[, er.list[i]]
  try(reg.temp <- lm(form.temp ~ 1), silent = TRUE)
  coef.temp <- data.frame(rep(0, 1))
  try(coef.temp <- as.data.frame(reg.temp$coefficients), silent = TRUE)
  exrtn.temp[i] <- coef.temp
  coef.sd <- list()
  coef.sd$coeftable <- matrix(c(0, 0, 0, 0), nrow = 1, ncol = 4)
  try(coef.sd <- summ(reg.temp), silent = TRUE)
  exp.rtn.sd[i] <- coef.sd$coeftable[1, 2]
  sd.temp[i] <- STDEV(capm.data[, er.list[i]])
  rm(reg.temp)
  rm(coef.sd)
}
exrtn.temp <- as.data.frame(as.numeric(exrtn.temp))
row.names(exrtn.temp) <- LIC.list
exp.rtn.sd <- as.data.frame(as.numeric(exp.rtn.sd))
row.names(exp.rtn.sd) <- LIC.list
sd.temp <- as.data.frame(as.numeric(sd.temp))
row.names(sd.temp) <- LIC.list
exrtn.temp <- merge(exrtn.temp, exp.rtn.sd, by = 0, all.x = TRUE, all.y = TRUE)
exrtn.temp <- merge(exrtn.temp, sd.temp, by.x = 1, by.y = 0, all.x = TRUE, all.y = TRUE)
names(exrtn.temp) <- c("Code", "Exp.Rtn", "Exp.Rtn.SE", "Rtn.SD")
LICs <- merge(LICs, exrtn.temp, by.x = 1, by.y = 1, all.x = TRUE, all.y = TRUE)
LICs$Exp.Rtn <- exrtn.temp[, 2]-((1+(1/100))^(1/12)-1) #Expected return less 1% annual management fee
LICs$Sharpe <- LICs$Exp.Rtn/LICs$Rtn.SD

# (2) Calculate returns of replication portfolios then E(x), V(x) & Sharpe Ratio for each
k <- nrow(data)
j <- nrow(LICs)
exrtn.temp <- as.numeric(vector())
exp.rtn.sd <- as.numeric(vector())
sd.temp <- as.numeric(vector())
shp.temp <- as.numeric(vector())
for(i in 1:j){
  ...
  reg.temp <- lm(rep.port.temp$Exc.Port.Rtn ~ 1)
  exrtn.temp[i] <- coef(reg.temp)
  coef.sd <- summ(reg.temp)
  exp.rtn.sd[i] <- coef.sd$coeftable[1, 2]
  sd.temp[i] <- STDEV(rep.port.temp[, 5])
}
LICs$RP.Exp.Rtn <- exrtn.temp
LICs$RP.Exp.Rtn.SE <- exp.rtn.sd
LICs$RP.Rtn.SD <- sd.temp
LICs$RP.Sharpe <- LICs$RP.Exp.Rtn/LICs$RP.Rtn.SD

# (3) Compare the performance of each LIC asset to that of the respective replication portfolio
LICs$'Lower Rep. Port. Risk?' <- ifelse(LICs$RP.Rtn.SD <= LICs$Rtn.SD, "TRUE", "FALSE")
